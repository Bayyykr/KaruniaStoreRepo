...
public class PopUp_transbelibuttontamabahbarangerror extends JDialog {
    ...
    public PopUp_transbelibuttontamabahbarangerror(JFrame parent) {
        ...
        warningTextLabel = createTextLabel("Error!", 150, 75, 150, 30, new Font("Arial", Font.BOLD, 18), Color.RED);
        warningTextLabel.setVisible(false);
        contentPanel.add(warningTextLabel);

        confirmTextLabel = createTextLabel(
                "<html><center>Please fill in the data purchase transaction column first!!</center></html>",
                25, 110, 400, 50, new Font("Arial", Font.PLAIN, 14), Color.BLACK);
        confirmTextLabel.setVisible(false);
        contentPanel.add(confirmTextLabel);

        backButton = createRoundedButton("Back", 160, 190, 130, 30, new Color(0xEBEBEB), Color.BLACK);
    ...
    private JButton createRoundedButton(String text, int x, int y, int width, int height, Color bgColor, Color textColor) {
        JButton button = new JButton(text) {
            ...
            protected void paintComponent(Graphics g) {
                ...
                g2.setColor(Color.BLACK);  // Border berwarna hitam
                g2.setStroke(new BasicStroke(1)); // Mengatur ketebalan border menjadi 1px
                g2.drawRoundRect(0, 0, getWidth() - 1, getHeight() - 1, 30, 30); // Gambar border
